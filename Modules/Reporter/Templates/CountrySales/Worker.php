...
$start = new \phpOMS\Datatypes\SmartDateTime('2015/07/01');
$end   = new \phpOMS\Datatypes\SmartDateTime('2015/07/31');
...
$cc = [
    ...
    'HUN' => 'HU', 'SRB' => 'XS', 'ALB' => 'AL', 'GRC' => 'GR', 'ROU' => 'RO', 'BGR' => 'BG',
    'EST' => 'EE', 'LVA' => 'LV', 'LTU' => 'LT', 'UKR' => 'UA', 'BLR' => 'BY', 'HRV' => 'HR',
    'SVN' => 'SI', 'CZE' => 'CZ', 'SVK' => 'SK', 'LIE' => 'LI', 'MKD' => 'MK', 'BIH' => 'BA',
    'LUX' => 'LU', 'MNE' => 'YU', 'JPN' => 'JP', 'CAN' => 'CA', 'USA' => 'US', 'MEX' => 'MX',
    'GTM' => 'GT', 'PRI' => 'PR', 'KOS' => 'XK',
    'AAA' => '???', 'Misc' => 'Misc',
];

$cc_developed = [
    'BEL' => 'BE', 'BGR' => 'BG', 'CZE' => 'CZ', 'DNK' => 'DK', 'DEU' => 'DE', 'EST' => 'EE',
    'GRC' => 'GR', 'ESP' => 'ES', 'FRA' => 'FR', 'HRV' => 'GR', 'ITA' => 'IT', 'CYP' => 'CY',
    'LVA' => 'LV', 'LTU' => 'LT', 'LIX' => 'LU', 'HUN' => 'HU', 'MLT' => 'MT', 'NLD' => 'NL',
    'AUT' => 'AT', 'POL' => 'PL', 'PRT' => 'PT', 'ROU' => 'RO', 'SVN' => 'SI', 'SVK' => 'SK',
    'FIN' => 'FI', 'SWE' => 'SE', 'GBR' => 'GB', 'ISL' => 'IS', 'NOR' => 'NO', 'CHE' => 'CH',
    'LIE' => 'LI', 'JPN' => 'JP', 'CAN' => 'CA', 'USA' => 'US', 'AUS' => 'AU',
];
...
while (($line = fgetcsv($file, 0, ';')) !== false) {
    $clients[$line[0]]['cc'] = $line[1];
    $clients[$line[0]]['value'] = 0.0;
    $clients[$line[0]]['id'] = $line[0];
}
fclose($file);
...
while (($line = fgetcsv($file, 0, ',', '"')) !== false) {
    ...
    if (isset($accounts[$line[10]]) && $date->getTimestamp() >= $start->getTimestamp() && $date->getTimestamp() <= $end->getTimestamp()) {
        $line[4] = (float) str_replace(['.', ','], ['', '.'], $line[4]);
        $line[3] = (float) str_replace(['.', ','], ['', '.'], $line[3]);
        $accounts[$line[10]] += $line[4] - $line[3];

        if(isset($clients[$line[11]])) {
            if(!isset($clients[$line[11]]['value'])) {
                $clients[$line[11]]['value'] = 0.0;
            }

            $clients[$line[11]]['value'] += $line[4] - $line[3];
        }

        if (isset($clients[$line[11]]) && !in_array($line[10], $ac_inland)) { // Client existiert und Erloeskonto ist fuers Inland
            if ($clients[$line[11]]['cc'] == 'QU') {
                $clients[$line[11]]['cc'] = '???';
            } // QU Ausland!
            if ($clients[$line[11]]['cc'] == 'DE') {
                $clients[$line[11]]['cc'] = '???';
            } // DE Muss aber Ausland sein!

            if (isset($countries[$clients[$line[11]]['cc']])) {
                $countries[$clients[$line[11]]['cc']] += $line[4] - $line[3];
            } else {
                $countries['Misc'] += $line[4] - $line[3];
            }

            if ($clients[$line[11]]['cc'] == '???') {
                $unknown[] = $line;
            }
...
foreach($cc_developed as $key => $value) {
    $developed['total'] += $countries[$value];
}

$undeveloped['total'] += $sum_countries - $developed['total'] - $sum_unknown;
...
foreach($clients as $key => $value) {
    if(count($developed['top']) >= $limit && count($undeveloped['top']) >= $limit) {
        break;
    }

    if(in_array($value['cc'], $cc_developed) && count($developed['top']) < $limit) {
        $developed['top'][] = $value;
    } elseif(!in_array($value['cc'], $cc_developed) && count($undeveloped['top']) < $limit) {
        $undeveloped['top'][] = $value;
    }
}
