    ...
    <div>
        <form method="post"
            ...
            <ul class="l-1">
                ...
                <li><label
                        for="i-areamanager"><?= $lang['Status']; ?></label>: <?= abs($sum_countries - $sum_accounts) < 0.01 && abs($sum_unknown - $countries['???']) < 0.01 && abs($developed['total']+$undeveloped['total']+$countries['???'] - $sum_accounts) < 0.01 ? 'OK' : 'NOK'; ?>
            </ul>
        </form>
    </div>
</div>
...
<div class="b b-2">
    <h1><?= $lang['Developed']; ?></h1>

    <div class="bc-1">
        <table class="tc-1">
                <tr>
                    <th><?= $lang['Total']; ?>
                    <td colspan="2"><?= number_format($developed['total'], 2, ',', '.'); ?>
                <?php foreach($developed['top'] as $client) : ?>
                    <tr>
                        <th><?= $client['id']; ?>
                        <td><?= number_format($client['value'], 2, ',', '.'); ?>
                        <td><?= $client['cc']; ?>
                <?php endforeach; ?>
        </table>
    </div>
...
<div class="b b-2">
    <h1><?= $lang['Undeveloped']; ?></h1>

    <div class="bc-1">
        <table class="tc-1">
                <tr>
                    <th><?= $lang['Total']; ?>
                    <td colspan="2"><?= number_format($undeveloped['total'], 2, ',', '.'); ?>
                <?php foreach($undeveloped['top'] as $client) : ?>
                    <tr>
                        <th><?= $client['id']; ?>
                        <td><?= number_format($client['value'], 2, ',', '.'); ?>
                        <td><?= $client['cc']; ?>
