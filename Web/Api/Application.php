    ...
    public function run($request, $response)
    {
        ...
        if (($uris = $request->getUri()->getQuery('r')) !== null) {
            ...
            foreach ($uris as $key => $uri) {
                ...
                $this->app->moduleManager->loadLanguage($response->getL11n()->getLanguage(), 'api');
            ...
            if ($request->getPath(2) === 'login' && $account->getId() < 1) {
                $response->setHeader('Content-Type', \phpOMS\System\MimeType::M_JSON . '; charset=utf-8', true);

                $login = $account->login($request->getData('user'), $request->getData('pass'));
                if ($login === \phpOMS\Auth\LoginReturnType::OK) {
                    $response->set($request->__toString(), new \Model\Message\Reload());
                } else {
            ...
            //$this->app->dispatcher->dispatch($this->app->router->route($request->getRoutify()), $request, $response, null);
            $this->app->moduleManager->get('Content')->call($request, $response);
        }
    }
}
