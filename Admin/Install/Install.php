 ...
 * @since      1.0.0
 */
class Install extends \phpOMS\ApplicationAbstract
{
    /**
     * Database object.
     *
    ...
    public function installCore()
    {
        switch ($this->dbPool->get('core')->getType()) {
            case \phpOMS\DataStorage\Database\DatabaseType::MYSQL:
                ...
                $this->dbPool->get('core')->con->prepare(
                    'CREATE TABLE if NOT EXISTS `' . $this->dbPool->get('core')->prefix . 'account_permission` (
                ...
                $this->dbPool->get('core')->con->prepare(
                    'ALTER TABLE `' . $this->dbPool->get('core')->prefix . 'account_permission`
                            ADD CONSTRAINT `' . $this->dbPool->get('core')->prefix . 'account_permission_ibfk_1` FOREIGN KEY (`account_permission_account`) REFERENCES `' . $this->dbPool->get('core')->prefix . 'account` (`account_id`);'
                )->execute();
